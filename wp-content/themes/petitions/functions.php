 ...
 *
 * @package origin-child
 * @subpackage Functions
 */

/* Adds the child theme setup function to the 'after_setup_theme' hook. */
add_action( 'after_setup_theme', 'origin_child_theme_setup', 11 );
 ...
 * @since 0.1.0
 */
function origin_child_theme_setup() {

	/* Get the theme prefix ("origin"). */
	$prefix = hybrid_get_prefix();

	/* Example action. */
	// add_action( "{$prefix}_header", 'origin_child_example_action' );

	/* Example filter. */
	// add_filter( "{$prefix}_site_title", 'origin_child_example_filter' );
