...
v.17.50
    FZSM: This version is mainly preparing the future 1.8.x version so I added some new improvements.
    * My collegue Sergei will continue working in some other areas, so I will manage the whole plugin from now on.
    * I extended the template system to allow javascript and css run independent for each module.
      This will be a huge difference, allowing to extend plugin without modifying headers or functions
    * First AJAX test were VERY promising. Even though I'm working to manage all the javascript using a framework,
     because the potential complexity. I added a new module ajaxSecond, currently disabled (but working).
    * Improved safe_ini mode evaluation. That should fix a problem with button migration is disabled when
      a statpress table is present.
    * Improved error control when Multibyte String Extension is not installed on Apache Server.
    * Improved running Statcomm behing a proxy. Code improvement thanks rogst
