...
v1.7.60
    ...
    * Corrected metabox expanding too much to the right.
    * Fixed an incorrect url reference on uaparser service.
    * fixed problem with gethostbyaddr
    * Reverse proxy fix http://wordpress.org/support/topic/plugin-statcomm-statpress-community-multisite-edition-correct-ip-address-behind-reverse-proxy? thanks to rogst
    * Shortcodes subplugin (in beta)
    * Maxmind database can be enabled/disabled manually on single and multisite environments
    * User Agent String database can be enabled/disabled manually on single and multisite environments.
    * Fixed warning on subplugin tabs.
    * Added Service authentication on both sides (plugin and server)
    * Added additional checking to be sure User Agent String database is in good condition.
    * Reports reconfigured dinamically if the user enables or disables Maxmind/UAS databases.
    * Translation to Spanish up-to-date
    * Corrected incorrect link when downloading Maxmind on Multisite environment.
    * Urls now are correctly decoded in every view using urls.
*/
  ...
  class statPressCommunity
  {
   ...
   function __construct()
   {
       ...
       //v1.7.50: datatables
        //add_action('wp_ajax_datatable_action', array(&$this, 'datatableQuery') );//data returned for tables.
       // add_action('wp_ajax_datatable_action', 'ajaxSecond::datatableQuery' );//data returned for tables.
...
//1.7.00
      ...
      static function jsScriptsPage()
      {
          ...
          //wp_enqueue_script('sc-datatable', plugin_dir_url(__FILE__) . 'js/sc-datatable.js', array('jquery'), false, true);
    ...
    function statcommUASUpdate()
    {
      ...
      /**
       * v1.7.50:Used to get Json data to display on the overview.
       * First version using datatables.net
       */
      function datatableQuery()
      {
          $date= isset($_GET['date'])?$_GET['date']:"";
          $query = isset($_GET['query'])?$_GET['query']:"";
          $aColumns = array( 'date', 'time', 'ip', 'urlrequested', 'referrer' );
          $sIndexColumn = "id";
 ...
 */
      ...
      function statcommMainOLD2()
      {
          ...
          jQuery(document).ready( function($) {
              ...
              // postboxes setup
              postboxes.add_postbox_toggles('<?php echo $this->currentPage; ?>');
          });
		//]]>
	</script>
      <?php
...

{
    ...
    return $res;
	  ...
	  public function statcommAppend()
      {
        ...
        //Thanks to: rogst
        //Correction when wp is behind a reverse proxy.
       ...
       //utilities::fl("Status code:",$statuscode);
        ...
        $ud=isset($userdata->user_login)?$userdata->user_login:"";
          ...
          //Note in this case subplugin shortcode will return nothing!!!
          if(empty($scu)) return '';
